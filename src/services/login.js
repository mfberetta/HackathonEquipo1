...
export default {
    register: async(usuarioSignup) => {
        ...
        usuario = await usuariosDao.getByQuery({ 'email': email })
        if (usuario.length != 0) {
            return null
        }
        ...
        user.password = createHash(usuarioSignup.password)
    ...
    login: async(usuarioLogin) => {
        ...
        if (email == config.admin.username) {
            ...
            config.setUserRol('admin')
        } else {
            user = await usuariosDao.getByQuery({ 'email': email })
            if (user.length == 0) {
                return null
            }
